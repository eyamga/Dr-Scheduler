...
class MathTask:
    ...
    As a contiguous time interval, i.e., there is no hole in it, it has a start_date and an
    ...
    def y_var(self, physician):
        ...
        """
        return self.y_vars[
            (self.name, self.start_date, self.end_date, physician)
        ]
...
class TaskMatcher:
    ...
    def _score_physicians(self, eligible_physicians: List[str], task: Any, period: Dict[str, Any], month: int) -> Dict[str, float]:
        scores = {}
    ...
    def update_physician_stats(self, physician: str, task: Any, period: Dict[str, Any]):
        self.physician_task_counts[physician][task.name] += 1
        self.physician_task_days[physician][task.name].extend(period['days'])
        self.physician_task_counts[physician]['last_task'] = task

        if task.is_call_task:
            self.physician_call_counts[physician][period['month']] += 1
...
class MathSchedule:
        ...
        for physician, task_list in schedule.items():
            ...
            for task_index, task_dict in enumerate(task_list):
                ...
                for i in range(len(days) - 1):
                    assert date.fromisoformat(days[i + 1]) == date.fromisoformat(days[i]) + datetime.timedelta(
                        days=1), f"Dates in 'days' are not continuous for {task_number_and_physician_str}!"
        ...
        try:
            for physician, task_list in self.schedule.items():
                for task_index, task_dict in enumerate(task_list):
                    task = task_dict['task']
                    vars.append(
                        self.y[(task.name, task_dict['start_date'], task_dict['end_date'], physician)].Index())
                    values.append(1)
    ...
    def _get_periods_days(self, week_periods):
        ...
        Returns:
            (A, B) with A (MAIN) and B (CALL) list of list of continuous days.
        """
        main_periods_days = []
        call_periods_days = []
    ...
    def _get_all_math_tasks_per_task(self, periods):
        ...
        Returns:
            A dict with entries [task.name] = [MathTask1, MathTask2, MathTask3, ...]. All tasks are non-overlapping and
    ...
    def _math_create_variables(self, periods):
        ...
        A variable corresponds to the assignment or not of one physician to a `MathTask`. Each variable is
        ...
        """
        self.y = {}  # y[("CTU_A", start_date, end_date, physician)]
                    ...
                    if task_category.days_parameter == TaskDaysParameter.DISCONTINUOUS:
                        ...
                        for index, main_period_days in enumerate(main_periods_days):
                            ...
                            for physician in all_physicians:
                                var_key = (task.name, start_date, end_date, physician)
                                self.y[var_key] = self.math_model.NewBoolVar(
                ...
                for index, math_task in enumerate(self.math_tasks[task.name][week_start]):
                    ...
                    for physician in all_physicians:
                        if physician not in available_physicians:
                            self.math_model.Add(
                                self.y[
                                    (task.name, math_task.start_date, math_task.end_date, physician)
                                ]
                                == 0
                            )
    ...
    def _math_create_call_linked_to_main_tasks_constraints(self, periods):
        """
        Construct constraints linking CALL tasks to MAIN tasks.
        ...
        def limit_number_of_call_math_tasks(C, physicians, max_nbr_linked_call_math_tasks=1):
            ...
            for physician in physicians:
                self.math_model.Add(
                    sum(C[i].y_var(physician) for i in range(len(C))) <= max_nbr_linked_call_math_tasks
                )
        ...
        def link_main_and_call_math_tasks(M, C, physicians):
            ...
            # if a physician is doing a MAIN task, they must do a CALL task too
            for physician in physicians:
                for i in range(len(M)):
                    if len(C) > 0:
                        self.math_model.Add(M[i].y_var(physician) <= sum(C[j].y_var(physician) for j in range(len(C))))

            # if a physician is doing a CALL task, they must do the corresponding MAIN tasks too
            for physician in physicians:
                for j in range(len(C)):
                    if len(M) > 0:
        ...
        for task in all_tasks_list:
            if task.type == TaskType.MAIN:
                ...
                bundled_call_tasks = []
                for week_start, week_periods in periods.items():
                    if task.name not in self.math_tasks or week_start not in self.math_tasks[task.name]:
                        continue
                    bundled_main_tasks.extend(self.math_tasks[task.name][week_start])

                    if linked_call_task_name and linked_call_task_name in self.math_tasks and week_start in self.math_tasks[linked_call_task_name]:
                        bundled_call_tasks.extend(self.math_tasks[linked_call_task_name][week_start])

                    number_of_weeks_left -= 1
                    if number_of_weeks_left == 0:
    ...
    def _math_create_non_simultaneous_tasks(self, periods):
        """
        Forbid one physician to do two simultaneous `MathTask`s.

        Args:
            periods:

        Notes:
            To allow for simultaneous tasks, simply remove those tasks from the list of `MathTask`s.
            For more complex tasks, rewrite this method and `_create_mutually_exclusive_math_tasks_constraints` accordingly.
        """
    ...
    def _create_mutually_exclusive_math_tasks_constraints(self, A, B):
        ...
        while i < len(A) and j < len(B):
            ...
            if lo <= hi:
                # both intervals intersect => add a mutually exclusive constraint for all physicians
                for physician in all_physicians:
    ...
    def _math_create_objective_function(self):
        ...
        """
        # TODO: Implement the actual objective function considering the scores
        self.math_model.Maximize(
            sum(
                self.y[key] * 1  # Placeholder coefficient
                for key in self.y
            )
        )
